...
1. 更改 main('a.cp2k', 'b.cp2k', switch='up')
    输出一系列文件夹，文件夹名字对应第二个晶体 ('b.vasp') 向上(a轴)移动的距离
    其中 a.cp2k 和 b.cp2k 对应需要合并的两个晶体文件名
2. 得到向上移动的最佳距离 x 时进行下一步
    更改 main('a.cp2k', 'b.cp2k', x, switch='forward')
    输出一系列文件夹，文件夹名字对应第二个晶体 ('b.vasp') 向前(b轴)移动的距离
3. 得到向前移动的最佳距离 y 时进行下一步
    更改 main('a.cp2k', 'b.cp2k', x, y, switch='center')
    输出一系列文件夹，文件夹名字对应中心晶界的距离
4. 得到中心晶界的最佳距离 z1 时进行下一步
    更改 main('a.cp2k', 'b.cp2k', x, y, z1, switch='border')
...
def main(inpa, inpb, upmove=0, forwardmove=0, centermove=0, bordermove=0, step=0.2, maxM=1.6, maxM_left=None, switch='up'):
    """
...
if __name__ == '__main__':
    main('a.cp2k','b.cp2k', 0, 0, switch='center', maxM=0.9,maxM_left=-0.8)
