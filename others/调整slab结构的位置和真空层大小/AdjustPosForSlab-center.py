"""
对于slab结构，调整原子位置，使之所有的原子都在中间（为了观察更方便），而且可以调整真空层厚度
"""
...
def adjust_pos(stru, dis_between_upborder=0, axis=2, vacuum=20):
...
def write_vasp(stru, dis_between_upborder=0, axis=2, filename="new.vasp"):
    # 让一层之内的原子不分开（加了真空层后可能导致在周期性最上方和最下方的原子分离）
    scaled_pos = stru.get_scaled_positions()
    scaled_pos[:,axis] += dis_between_upborder
    stru.set_scaled_positions(scaled_pos)
    stru.wrap(pbc=[True, True, True])  # 让周期性边界外的原子回到周期内
...
#    print(up_arr)
    ...
    # 如果分不开两类，则向下移动
    if flag:
        dis_between_upborder1 = -np.array(up_arr)[:,axis].min()
    else:
        dis_between_upborder1 = 1 - np.array(up_arr)[:,axis ].min()
    adjust_pos(stru_read, dis_between_upborder1, axis, vacuum=40)
    
    pos_axis = stru_read.get_scaled_positions()[:,axis]
    cluster_center = (pos_axis.max() - pos_axis.min())/2
    dis_between_upborder = 0.5 - cluster_center
    
    write_vasp(stru_read, dis_between_upborder, axis, filename="new1.vasp")