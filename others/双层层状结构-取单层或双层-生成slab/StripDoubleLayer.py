"""
作者：陈果
描述：取出两层层状结构的的双层，并加上20A的真空层（原子在底部，需要设置真空层所在轴），生成两个新的POSCAR，分别对应AB堆垛，和BA堆垛
"""
...
def classfied(arr, class_=3, axis=1):
    ...
    labels = np.where(labels==100, 0, labels) # 出现最少的元素替换成0
    
    return labels

def write_vasp(stru, dis_between_upborder=0, axis=1, filename="new.vasp"):
    ...
    len_atoms_max = stru.get_scaled_positions()[:,axis].max() * len_axis_vacuum
...
if __name__ == "__main__":
    ...
    write_vasp(stru_read_copy0, dis_between_upborder0, axis, "kind1.vasp")

    stru_read_copy1 = stru_read.copy()
    write_vasp(stru_read_copy1, dis_between_upborder1, axis, "kind2.vasp")