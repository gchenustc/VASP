...
BORN_NROWS=`grep NIONS OUTCAR-born | awk '{print $12*4+1}'`  
if [ `grep 'BORN' OUTCAR-born| wc -l` = 0 ] ; then printf " .. FAILED! Born effective charges missing! Bye! \n\n" ; exit 1 ; fi  
grep "in e, cummulative" -A $BORN_NROWS OUTCAR-born > born.txt  
...
for nu in `seq 1 $EIG_NVIBS` ; do  
 ...
 for alpha in 1 2 3 ;  do            # summing over alpha coordinates  
  ...
  for s in `seq 1 $EIG_NIONS` ; do   # summing over atoms  
   ...
   paste z.ion.$ss.alpha.$alpha  e.vib.$nuu.ion.$ss |   awk '{pol=$1*$4+$2*$5+$3*$6; print $0,"  ",pol}' > matr-vib-${nuu}-alpha-${alpha}-ion-${ss}  
  done  
...
printf "%5u atoms found\n%5u vibrations found\n%5u matrices evaluated"        $EIG_NIONS $EIG_NVIBS $NMATRIX > results/statistics.txt  
