...
class HollowCuboid(object):
    """
    获得 POSCAR 指定位置的 长方体 - 基于挖出长方体脚本的反转
    使用方法:
    1. 指定输入输出文件路径
    ...
    def getNotInCubicIndex(pos_arr, center_point, lattice_constant):
        ...
        print("在小胞中的原子索引(从1开始)",inbox_index)
        return ret
...
if __name__=="__main__":
    hc = HollowCuboid()
    hc.operate(center_point=[10.0,9.4,10.2], lattice_constant=[6.25,6.25,5.05], rotate_axis=[0,0,1], rotate_angle=np.pi/4)
    hc.retPoscar()
