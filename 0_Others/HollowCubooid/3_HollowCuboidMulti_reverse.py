...
class HollowCuboid(object):
    """
    获得 POSCAR 指定位置的 长方体 - 支持一次性获得多个长方体
    使用方法:
    1. 指定输入输出文件路径
    ...
    hc.operate(cuboids = [ [[5,5,5], [10,10,10], [0,0,1], np.pi/4], [...], [...] ])
    cuboids 是列表，每个列表中的元素也是列表，里面分别存放小胞的中心点，晶格常数，小胞旋转轴，小胞旋转角度
    3. 输出
    hc.retPoscar()
    """
        ...
        for index,i in enumerate(pos_arr):
            for each_coord in i:
                if HollowCuboid.isInBox(each_coord,x_range,y_range,z_range):
                    ret[index] = 0
                    inbox_index.append(index+1)
        return ret, inbox_index
    ...
    def operate(self, cuboids):
        """
        lattice_constant: 小立方体的晶格常数
        """
        inbox_index_total = []
        notinbox_ont_hot = np.ones(self.atoms_num)
        for cuboid in cuboids:
            
            center_point = cuboid[0]
            lattice_constant = cuboid[1]
            rotate_axis = cuboid[2]
            rotate_angle = cuboid[3]
            ...
            if rotate_axis:
                ...
                notinbox_ont_hot_,inbox_index = HollowCuboid.getNotInCubicIndex(pos_arr_rotate.reshape((-1,27,3)), center_point, lattice_constant)
            else:
                notinbox_ont_hot_,inbox_index = HollowCuboid.getNotInCubicIndex(pos_arr_expand.reshape((-1,27,3)), center_point, lattice_constant)
            
            inbox_index_total.extend(inbox_index)
            notinbox_ont_hot = notinbox_ont_hot * notinbox_ont_hot_
        
        inbox_index_set = set(inbox_index_total)
        print("在小胞中的原子索引:(从1开始)",inbox_index_set)

        # notinbox_ont_hot 反转 - 获得小胞内的原子
        f1 = np.frompyfunc(lambda x: 0 if x==1 else 1, 1, 1)
        inbox_ont_hot = f1(notinbox_ont_hot)
            
        # 改变 POSCAR 信息
        notinbox_index = np.argwhere(inbox_ont_hot==1).flatten()
        ...
        for index,i in enumerate(inbox_ont_hot):
            if i==0:
...
if __name__=="__main__":
    hc = HollowCuboid()
    hc.operate([ [[6, 6, 5],[6.25,6.25,5.05], [0,0,1], np.pi/4], [[10, 9, 8],[7,8,5.05], [0,0,1], np.pi/6] ])
    hc.retPoscar()
