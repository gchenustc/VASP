...
cohp_x = [-2,2] # COHP x 轴范围, 可选"auto"
cohp_y= [-27, 10]# COHP y 轴范围，可选"auto"
icohp_x = [-0.01, 15] # icohp x 范围， 不可选"auto"
cohp_sep_x = 1 # x轴刻度间隔
cohp_sep_y = 4 # y轴刻度间隔
...
for i in range(len_cal):
    ...
    if not isinstance(cohp_x, str) or cohp_x.lower() != "auto":
        ax1.set_xlim(cohp_x)  # 根据lobster的设置更改, lobster算了某个范围的能量后才有效果
    if not isinstance(cohp_y, str) or cohp_y.lower() != "auto":
    ...
    ax2.annotate(f'{-icohp_ef[i]:.3f}', xy=(icohp_x[1]-0.5, -0.08),
